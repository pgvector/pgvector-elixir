...
defmodule EctoTest do
  ...
  test "halfvec l2 distance" do
    items =
      Repo.all(from i in Item, order_by: l2_distance(i.half_embedding, ^Pgvector.HalfVector.new([1, 1, 1])), limit: 5)
  ...
  test "halfvec max inner product" do
    items =
      Repo.all(
        from i in Item, order_by: max_inner_product(i.half_embedding, ^Pgvector.HalfVector.new([1, 1, 1])), limit: 5
      )
  ...
  test "halfvec cosine distance" do
    items =
      Repo.all(
        from i in Item, order_by: cosine_distance(i.half_embedding, ^Pgvector.HalfVector.new([1, 1, 1])), limit: 5
      )
  ...
  test "halfvec cosine similarity" do
    items =
      Repo.all(
        from i in Item, order_by: 1 - cosine_distance(i.half_embedding, ^Pgvector.HalfVector.new([1, 1, 1])), limit: 5
      )
  ...
  test "halfvec l1 distance" do
    items =
      Repo.all(from i in Item, order_by: l1_distance(i.half_embedding, ^Pgvector.HalfVector.new([1, 1, 1])), limit: 5)
  ...
  test "sparsevec l2 distance" do
    items =
      Repo.all(
        from i in Item, order_by: l2_distance(i.sparse_embedding, ^Pgvector.SparseVector.new([1, 1, 1])), limit: 5
      )
  ...
  test "sparsevec max inner product" do
    items =
      Repo.all(
        from i in Item, order_by: max_inner_product(i.sparse_embedding, ^Pgvector.SparseVector.new([1, 1, 1])), limit: 5
      )
  ...
  test "sparsevec cosine distance" do
    items =
      Repo.all(
        from i in Item, order_by: cosine_distance(i.sparse_embedding, ^Pgvector.SparseVector.new([1, 1, 1])), limit: 5
      )
  ...
  test "sparsevec cosine similarity" do
    items =
      Repo.all(
        from i in Item,
          order_by: 1 - cosine_distance(i.sparse_embedding, ^Pgvector.SparseVector.new([1, 1, 1])),
          limit: 5
      )
  ...
  test "sparsevec l1 distance" do
    items =
      Repo.all(
        from i in Item, order_by: l1_distance(i.sparse_embedding, ^Pgvector.SparseVector.new([1, 1, 1])), limit: 5
      )
