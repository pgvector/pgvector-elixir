...
defmodule PostgrexTest do
  ...
  test "works", %{pid: pid} = _context do
    Postgrex.query!(pid, "INSERT INTO items (embedding) VALUES ($1), ($2), ($3)", [
      Pgvector.new([1, 1, 1]),
      Pgvector.new([2, 2, 2]),
      Nx.tensor([1, 1, 2], type: :f32) |> Pgvector.new()
    ])

    result =
      Postgrex.query!(pid, "SELECT * FROM items ORDER BY embedding <-> $1 LIMIT 5", [
        Pgvector.new([1, 1, 1])
      ])
  ...
  test "tensor", %{pid: pid} = _context do
    ...
    result = Postgrex.query!(pid, "SELECT $1::vector", [embedding |> Pgvector.new()])
  ...
  test "tensor rank", %{pid: pid} = _context do
    assert_raise ArgumentError, "expected rank to be 1", fn ->
      Postgrex.query!(pid, "SELECT $1::vector", [Nx.tensor([[1, 2, 3]]) |> Pgvector.new()])
    end
  end
end
